    ...
    , pQCefViewHandler_(nullptr)
  {
    ...
    if (!CefBrowserHost::CreateBrowser(window_info,       // window info
      ...
      throw std::exception("Failed to create browser.");
        ...
        qWarning() << "Success to create browser.";
    }
  }
    ...
    if (pQCefViewHandler_) {
      ...
      if (browser) {
        ...
        if (host) {
          ...
          if (!zoom) {
             zoom = true;
             host->SetZoomLevel(cefWindow()->devicePixelRatio());
          }
		}
      }
    }
...
void QCefView::onAfterCreated()
{

}

void QCefView::onBeforeClose()
{

}

bool QCefView::onBeforeResourceLoad(const QString &url, const QString &method, QMap<QString, QString> *headMap)
{
    return true;
}

void QCefView::uninitializeCef()
{
    if (pImpl_)
      pImpl_->uninitializeCef();

    CCefManager::getInstance().uninitializeCefOnce();
}
