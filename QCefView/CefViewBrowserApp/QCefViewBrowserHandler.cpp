...
QCefViewBrowserHandler::OnAfterCreated(CefRefPtr<CefBrowser> browser)
{
  ...
  if (pQCefViewDelegate_)
    pQCefViewDelegate_->onAfterCreated();
...
QCefViewBrowserHandler::OnBeforeClose(CefRefPtr<CefBrowser> browser)
{
  ...
  if (main_browser_ && main_browser_->IsSame(browser)) {
    ...
    if (pQCefViewDelegate_)
      pQCefViewDelegate_->onBeforeClose();
                                    ...
                                    const CefString& failedUrl)
{
  ...
  CefString title = "No Internet";
  CefString tryTitle = "Try";
  CefString tryText1 = "Checking the network cables, modem, and router";
  CefString tryText2 = "Reconnecting to Wi-Fi";
  unsigned short lang = GetUserDefaultLCID() & 0xFF;
  if (lang == LANG_CHINESE) {
//       title.FromString(QString::fromLocal8Bit("未联网").toStdString());
//       tryTitle.FromString(QString::fromLocal8Bit("请尝试").toStdString());
//       tryText1.FromString(QString::fromLocal8Bit("检查网络电缆、调制解调器和路由器").toStdString());
//       tryText2.FromString(QString::fromLocal8Bit("重新连接到无线网络").toStdString());
  }
  ...
  oss << "<html>"
      ...
      <<   "<div><h1>"
        ...
        << tryTitle
      <<    ":<br><ul>"
      <<    "<li style='list-style-type: disc;margin-left:40px;'>"
         <<  tryText1
      <<      "</li>"
       <<    "<li style='list-style-type: disc;margin-left:40px;'>"
          <<  tryText2
...
{
    int ret = true;
    CefRequest::HeaderMap headMap;
    request->GetHeaderMap(headMap);
    if (pQCefViewDelegate_) {
        ret = pQCefViewDelegate_->onBeforeResourceLoad(request->GetURL().ToString(), request->GetMethod().ToString(), &headMap);
    }
    if (!ret) {
        return RV_CANCEL;
    }
    if (headMap.size() > 0) {
        request->SetHeaderMap(headMap);
    }
...
//  pQCefViewDelegate_ = nullptr;// 不再响应
